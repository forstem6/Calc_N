...
ui <- fluidPage(
  ...
  fluidRow(
    hr(),
    h5("In dieser App wird nach Auswahl von spezifischen Parametern der benötigte Stichprobenumfang für eine vorausgewählte Reihe an Veranstaltungsgrößen berechnet."),
    h5("Nutzung: Die benötigten der Stichprobenumfänge hängen von einer Reihe an Faktoren ab. Dies sind"),
    tags$ul(tags$li("die Verteilung der Antworten in der Studierendenschaft (hier vereinfacht durch die Verteilung der Antworten 4 und 5 gegenüber den Antworten 1,2,3)",
                    tags$li("der zumutbare Zufallsfehler für den wahren Wert in der Population"),
                    tags$li("das Kofidenzintervall")
    ),
    h5("Nun können in den Feldern links die Werte eingegeben werden. Nach jeder Eingabe wird die Berechnung angepasst. Folgende Werte sind möglich:"),
    tags$ul(tags$li("Bei der Prozentzahl der Antworten 4,5 sind Werte von 0 - 100 möglich. Die Prozente der Antworten 1,2,3 ergeben sich aus den Angaben."),
            tags$li("Der Zufallsfehler kann 0 - 100 % betragen, wobei Werte unter 10 % üblich/sinnvoll sind."),
            tags$li("Das Konfidenzintervall kann 0 - 100 % betragen, wobei Werte über 90 % üblich/sinnvoll sind.")
            ),
    h5("Am Ende kann noch eine sebst gewählte Stichprobengröße eingegeben werden. Diese wird dann in der Tabelle hinzugefügt und in den Grafiken mit einer senkrechten Linie dargestellt.")    
    ),
  ),
  fluidRow(
